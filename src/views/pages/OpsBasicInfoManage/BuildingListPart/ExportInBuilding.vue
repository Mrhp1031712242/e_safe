<template>
  <div class="export_dia_wrap">
    <div class="top_txt">
      <div class="le_name fl" :style="{ color:!exportName ? 'rgba(255,255,255,0.5)': '#fff'}">{{exportName || "楼栋列表导入文件名"}}</div>
      <div class="ri_btn fr">
        <span style="display:inline-block;margin-left:10px;">
          <el-upload 
          :action="exportInUrl+'/api/device/building/importBuilding'"
          ...
          :show-file-list="false">
            ...
            <template #tip>
              <a :href="baseDownLoadURL + '/template/esafe/esafe_buildings.xls'" class="down_load_btn">下载模板</a>
            </template>
          </el-upload>
        </span>
      </div>
    ...
    <div class="table_part_wrap table_list_part">
      ...
      >
        ...
        <table-column prop="name" label="楼栋名称" />
        <table-column prop="address" label="具体地址" />
        <table-column prop="linkMan" label="楼栋负责人" />
...
import { buildingAddMore } from "@/api/requestData/opsBasicInfo";
  ...
  setup(props,ctx){
    ...
    const doPriceSubmit = (data)=>{
      ...
      data.forEach(item=>{
        ...
        obj.pmc = item.pmc || null;
        paramsData.push(obj);
      })
      buildingAddMore(paramsData).then(res => {
        if(res.code == import.meta.env.VITE_APP_API_SUCCESS_CODE){
          closeExport(true);
        }
      }).catch(error=>{
