<template>
  <div class="export_dia_wrap">
    <div class="top_txt">
      <div class="le_name fl" :style="{ color:!exportName ? 'rgba(255,255,255,0.5)': '#fff'}">{{exportName || "监测设备导入文件名"}}</div>
      <div class="ri_btn fr">
        <span style="display:inline-block;margin-left:10px;">
          <el-upload 
          :action="exportInUrl+'/api/device/device/importDevice'"
          ...
          :show-file-list="false">
            ...
            <template #tip>
              <a :href="baseDownLoadURL + '/template/esafe/esafe_devs.xls'" class="down_load_btn">下载模板</a>
            </template>
          </el-upload>
        </span>
      </div>
    ...
    <div class="table_part_wrap table_list_part">
      ...
      >
        ...
        <table-column prop="id" label="监测设备ID"/>
        <table-column prop="hardwareV" label="硬件版本"/>
        <table-column prop="softwareV" label="软件版本"/>
        <table-column prop="realtime" label="实时上报" optional>
          <template #default="scope">
            <span>{{scope.row.realtime == false ? '否' : '是' }}</span>
          </template>
        </table-column>
        <table-column prop="sensorPeriod" label="上报周期"/>
        <!-- <table-column prop="deviceModelName" label="产品型号"/> -->
        <!-- <table-column prop="deviceModelName" label="产品型号"/> -->
        <!-- <table-column prop="versionType" label="固件版本"/> -->
        <!-- <table-column prop="IMEI" label="IMEI"/> -->
        <table-column prop="linkMan" label="设备负责人" width="100"/>
        <table-column prop="phone" label="手机号"/>
        <table-column prop="areaStr" label="区域"/>
        <table-column prop="villageName" label="小区/村居" width="100"/>
        <table-column prop="buildingName" label="楼栋"/>
        <table-column prop="roomName" label="房间"/>
        <table-column prop="address" label="安装位置"/>
...
import { moniDevAddMore } from "@/api/requestData/opsBasicInfo"
  ...
  setup(props,ctx){
    ...
    const doPriceSubmit = (data)=>{
      ...
      data.forEach(item=>{
        let obj = {};
        obj.id = item.id || null;
        obj.deviceTypeId = item.deviceTypeId || null;
        obj.hardwareV = item.hardwareV || null;
        obj.softwareV = item.softwareV || null;
        obj.protocolVersion = item.protocolVersion || null;
        obj.sensorPeriod = item.sensorPeriod || 60;
        obj.realtime = item.realtime || false;
        obj.linkMan = item.linkMan || null;
        obj.phone = item.phone || null;
        obj.areaId = item.areaId || null;
        obj.villageId = item.villageId || null;
        obj.buildingId = item.buildingId || null;
        obj.roomId = item.roomId || null;
        obj.address = item.address || null;
        paramsData.push(obj);
      })
      moniDevAddMore(paramsData).then(res => {
        if(res.code == import.meta.env.VITE_APP_API_SUCCESS_CODE){
          closeExport(true);
        }
      }).catch(error=>{
