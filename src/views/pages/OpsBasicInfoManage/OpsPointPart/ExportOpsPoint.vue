<template>
  <div class="export_dia_wrap">
    <div class="top_txt">
      <div class="le_name fl" :style="{ color:!exportName ? 'rgba(255,255,255,0.5)': '#fff'}">{{exportName || "监测点导入文件名"}}</div>
      <div class="ri_btn fr">
        <span style="display:inline-block;margin-left:10px;">
          <el-upload 
          :action="exportInUrl+'/api/device/deviceMonitor/importDeviceMonitorList'"
          ...
          :show-file-list="false">
            ...
            <template #tip>
              <a :href="baseDownLoadURL + '/template/esafe/esafe_ops_points.xls'" class="down_load_btn">下载模板</a>
            </template>
          </el-upload>
        </span>
      </div>
    ...
    <div class="table_part_wrap table_list_part">
      ...
      >
        ...
        <table-column prop="villageName" label="小区/村居"/>
        <table-column prop="buildingName" label="楼栋"/>
        <table-column prop="roomName" label="房间"/>
        <table-column prop="monitorName" label="监测点名称" />
        <table-column prop="deviceId" label="监测设备ID" />
        <table-column prop="port" label="端口" />
        <table-column prop="meterId" label="电表编号" />
...
import { moniPointAddMore } from "@/api/requestData/opsBasicInfo"
  ...
  setup(props,ctx){
    ...
    const doPriceSubmit = (data)=>{
      ...
      data.forEach(item=>{
        ...
        obj.monitorName = item.monitorName || null;
        obj.deviceId = item.deviceId || null;
        obj.type = item.type || null;
        obj.port = item.port || null;
        obj.meterId = item.meterId || null;
        paramsData.push(obj);
      })
      moniPointAddMore(paramsData).then(res => {
        if(res.code == import.meta.env.VITE_APP_API_SUCCESS_CODE){
          closeExport(true);
        }
      }).catch(error=>{
