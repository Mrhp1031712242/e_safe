<template>
  <div class="export_dia_wrap">
    <div class="top_txt">
      <div class="le_name fl" :style="{ color:!exportName ? 'rgba(255,255,255,0.5)': '#fff'}">{{exportName || "房间导入文件名"}}</div>
      <div class="ri_btn fr">
        <span style="display:inline-block;margin-left:10px;">
          <el-upload 
          :action="exportInUrl+'/api/device/room/importRoom'"
          ...
          :show-file-list="false">
            ...
            <template #tip>
              <a :href="baseDownLoadURL + '/template/esafe/esafe_rooms.xlsx'" class="down_load_btn">下载模板</a>
            </template>
          </el-upload>
        </span>
      </div>
    ...
    <div class="table_part_wrap table_list_part">
      ...
      >
        ...
        <table-column prop="buildingName" label="楼栋名称" />
        <table-column prop="name" label="房号" />
...
import { roomAddMore } from "@/api/requestData/opsBasicInfo"
  ...
  setup(props,ctx){
    ...
    const doPriceSubmit = (data)=>{
      ...
      data.forEach(item=>{
        ...
        obj.owner = item.owner || null;
        obj.phone = item.phone || null;
        paramsData.push(obj);
      })
      roomAddMore(paramsData).then(res => {
        if(res.code == import.meta.env.VITE_APP_API_SUCCESS_CODE){
          closeExport(true);
        }
      }).catch(error=>{
